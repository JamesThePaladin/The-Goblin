...
- [x] **Side-by-side pace check against an actual Rivulet in co-op — PASSED 2026-07-19.** Sanctus kept up. **Feature 1 is DONE.** (Note it was re-verified *after* the weight fix landed, so this was judged at the correct 0.28 mass, not the earlier Survivor weight.)
...
### ✅ Task 0 — lizard code READ 2026-07-19. Verdict: mechanism is portable, movement is NOT.

**How a lizard actually sticks to a wall — it's disarmingly simple.** `Lizard.Update` branches on
a bool field `applyGravity`:

```csharp
if (applyGravity) {
    airFriction = 0.999f;  gravity = 0.9f;
    bounce = 0.1f;  surfaceFriction = 0.3f;
} else {
    airFriction = 0.8f;   gravity = 0f;      // <-- climbing. That's the whole trick.
    bounce = 0.1f;
}
```

`gravity` is a settable property on `PhysicalObject`, so **this is fully available to `Player`.**
There is no special "climb" physics — a climbing lizard is just an object with gravity switched
off whose limbs are holding terrain.

**The supporting cast:**
- `LizardLimb` (extends the same `BodyPart`/`Limb` family as the player's `SlugcatHand`) carries
  `reachingForTerrain`, `gripCounter`, `grabPos`, `currentlyDisabled` — the limbs pick terrain
  points and hold them.
- The gravity decision uses hysteresis, not an instant test: `inAllowedTerrainCounter`,
  `NoGripCounter`, and `LizardBreedParams.regainFootingCounter`. **Copy this idea** — a raw
  "is there a wall next to me" check will flicker on and off at tile boundaries.

**⚠ The blocker for "rip it exactly": lizard movement is generated by PATHFINDING, not physics.**
Lizards move along `MovementConnection`s drawn from the room's AI map, which encodes per-creature
traversability. A `Player` is directly controlled and has no AI path, so there is nothing to port
— the part of lizard traversal that makes it look effortless is the AI choosing good grip
sequences in advance.

**So the plan's "rip lizard traversal exactly if feasible" is NOT feasible**, and that should be
settled now rather than discovered halfway in. What IS feasible, and should give the same feel:

1. Detect climbable terrain adjacent to the player (`Room.Tile.Terrain == Solid`).
2. While gripping, set `player.gravity = 0f` — the exact lizard mechanism.
3. Project player input onto the surface tangent so he runs along walls/ceilings at full speed
   (this is where "no speed cost" is won or lost).
4. Gate entry/exit on a counter with hysteresis, mirroring `inAllowedTerrainCounter`.

That is a physics-affordance design, not an AI port. It keeps the Goblin *player-controlled*,
which is what we actually want — the player chooses the route, the physics simply allows it.

### ✅ Step 1 DONE 2026-07-19, verified in-game — `src/GoblinWallWalk.cs`
Clinging works. Held on **SPECIAL** (`input[0].spec`, the button the Watcher's ability uses), so
normal movement is untouched unless the player asks for it.

**Two mistakes worth not repeating:**

1. **Setting `self.gravity = 0f` from a post-`orig` hook does nothing.**
   `PhysicalObject.gravity` is `{ get => g * room.gravity; set => g = value; }` and
   `Player.Update` assigns it every frame, **consuming it inside `orig`**. The assignment is
   overwritten before it can apply. Instead, add the applied gravity back onto velocity
   (`chunk.vel.y += self.gravity`) — order-independent, and leaves no state to reset on release.
   The first version did the property thing and the suspension never ran at all; everything that
   *seemed* to work was the drag.
2. **Never blanket-damp velocity.** `chunk.vel *= drag` every frame reads as wading through glue
   AND silently destroys jump height. Friction must apply **only to axes with no steering input**
   (`if (!steeringX) vel.x *= friction`) — grip when not pushing, free when pushing, exactly like
   ground friction. Without any friction he skates around on momentum like he's on ice; with
   blanket friction he can't jump. Input-aware is the only version that feels right.

**Mid-air guard (the thing Sanctus was rightly worried about):** clinging requires either
`Tile.wallbehind` on the occupied tile — the "climbable background piece" — or a `Solid`/`Slope`
tile in one of the four neighbours. Plain air with nothing behind it is rejected, so he cannot
hang over a pit.
